...
description: Wire dependency-cruiser into a TypeScript repo so package root files are its only public entry points and subfolders stay private.
...
For the vocabulary (deep module, interface, seam, depth), use the repository's existing architecture guidance when available.
...
2. **Intra-package freedom** — a package's own non-test files import each other freely.
...
4. **Tests folder is private** — code outside a package's `tests/` directory cannot import its test fixtures.
...
- **Existing config** — check for a `.dependency-cruiser.*` file. If one exists, do **not** overwrite it: merge the rules and options in, and tell the user what you added.
...
- Fold it into the repo's umbrella check command — the one that already runs typecheck (for example, `check`, `ci`, or `validate`). Do **not** touch `tsconfig` or add path aliases.
